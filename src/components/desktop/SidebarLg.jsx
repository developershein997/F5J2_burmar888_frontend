...
const SidebarLg = ({ show, onClose }) => {
  ...
  return (
     ...
     <>
      <div
          className={`fixed right-0 top-1 -translate-y-1/2 bg-[#181c2f] z-50 
    ...
    flex flex-col w-80 max-w-full h-auto rounded-lg shadow-lg  `}
      ...
      {/* Sidebar Drawer */}
      <div
          className={`fixed top-0 right-0 w-80 max-w-full  rounded-lg bg-[#181c2f] z-50 transition-transform duration-300
    ${show ? 'translate-x-0' : 'translate-x-full'} flex flex-col h-auto`}
      >
        ...
        {Number(user?.balance).toLocaleString()} Ks
            ...
            {contacts &&
                contacts.map((contact, index) => (
                    <Link to="/contact" key={index} onClick={onClose}>
                      <img
                          src={`https://ag.burmar888.online/api/..${contact.image}`}
          ...
          <button
              ...
              className="w-full py-2 bg-red-600 hover:bg-red-700 text-white font-bold text-lg rounded-b-xl transition-colors mt-2 mb-4"
          >
            {loading ? (
